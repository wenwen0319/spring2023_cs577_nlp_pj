...
class Instructor:
    def __init__(self, opt):
        ...
        self.train_data_loader = BucketIterator(data=absa_dataset.train_data, batch_size=opt.batch_size, shuffle=True, get_idx = 0)
        self.test_data_loader = BucketIterator(data=absa_dataset.test_data, batch_size=opt.batch_size, shuffle=True, get_idx = 1)
        # self.test_data_loader = BucketIterator(data=absa_dataset.test_data, batch_size=opt.batch_size, shuffle=False, sort=False)

        # changed by wenwen
        mat_save = {}
        mat_save['graph_all'] = self.test_data_loader.graph_list

        scipy.io.savemat(('./graph_list_{}.mat'.format(opt.dataset)), mat_save)
                ...
                if global_step % self.opt.log_step == 0:
                    ...
                    if test_f1 > max_test_f1:
                        ...
                        if self.opt.save and test_f1 > self.global_f1:
                            print('----------------------  ', self.opt.save, test_f1, self.global_f1,
                                  (test_f1 > self.global_f1), self.opt.save and (test_f1 > self.global_f1),
                                  self.opt.save and test_f1 > self.global_f1)
    ...
    def _evaluate_acc_f1(self):
        ...
        with torch.no_grad():
            for t_batch, t_sample_batched in enumerate(self.test_data_loader):
                ...
                _, _, _, t_graph = t_inputs  # extract the graph
                t_graph_value = t_graph.data.cpu().numpy()
                ...
                if t_targets_all is None:
                    ...
                    # t_graph_all = torch.cat((t_graph_all, t_graph), dim=0)
        ...
        t_targets_numpy = t_targets_all.cpu().data.numpy()
        t_outputs_numpy = torch.argmax(t_outputs_all, -1).cpu().data.numpy()
        t_correct_numpy = t_correct_all.cpu().data.numpy()

        # concat the numpy matrix
        # t_targets_numpy = t_targets_numpy.T
        # t_outputs_numpy = t_outputs_numpy.T
        # t_correct_numpy = t_correct_numpy.T

        t_result_all_numpy = numpy.vstack((t_targets_numpy, t_outputs_numpy))
        t_result_all_numpy = numpy.vstack((t_result_all_numpy, t_correct_numpy))
        t_result_all_numpy = t_result_all_numpy.T

        # save the file
        if not os.path.exists('case_study/'):
            os.mkdir('case_study/')
        # f_out = open('log/'+self.opt.model_name+'_'+self.opt.dataset+'_val.txt', 'w', encoding='utf-8')
        # f_out = open('case_study/' + 'predict1.txt', 'w', encoding='utf-8')

        # case_correct_save_name = 'case_study/case_correct_LSTM_' + self.opt.dataset + '.txt'
        # numpy.savetxt(case_correct_save_name, t_result_all_numpy, fmt = "%.5f %.5f %.5f", delimiter = "\n")


        # # ===========================
        # # save the all test sentences
        # # ===========================
        # # arrange the all sentences
        # test_sentence_all = ''
        # for test_sample_loop in range(n_test_total):
        #     raw_sentence_list = self.raw_text_output['test'][test_sample_loop]
        #     test_sentence_all = test_sentence_all + raw_sentence_list
        #
        # # save the all sentences
        # test_save_name = './case_study/test_' + self.opt.dataset + '.txt'
        # fo = open(test_save_name, "w")
        # fo.write(test_sentence_all)
        # fo.close()
        # # ===========================
    ...
    #     print(t_targets_all.cpu().data.numpy(), torch.argmax(t_outputs_all, -1).cpu(), n_test_total)
    #     # print(t_targets_all.cpu(), len(torch.argmax(t_outputs_all, -1).cpu()), n_test_total)
    #     # print(self.raw_text_output['test'][1])
    #
    #
    #     test_acc = n_test_correct / n_test_total
    #     f1 = metrics.f1_score(t_targets_all.cpu(), torch.argmax(t_outputs_all, -1).cpu(), labels=[0, 1, 2], average='macro')
