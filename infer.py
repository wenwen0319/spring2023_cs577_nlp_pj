...
class Inferer:
    ...
    def evaluate(self, raw_text, aspect):
        ...
        print(dependency_adj_matrix(raw_text.lower()))
...
if __name__ == '__main__':
    ...
    model_state_dict_paths_lstm = {
        'lstm': 'state_dict/LSTM_model/lstm_'+dataset+'.pkl',
        'ascnn': 'state_dict/LSTM_model/ascnn_'+dataset+'.pkl',
        'asgcn': 'state_dict/LSTM_model/asgcn_'+dataset+'.pkl',
    }

    model_state_dict_paths_nolstm= {
        'lstm': 'state_dict/noLSTM_model/lstm_'+dataset+'.pkl',
        'ascnn': 'state_dict/noLSTM_model/ascnn_'+dataset+'.pkl',
        'asgcn': 'state_dict/noLSTM_model/asgcn_'+dataset+'.pkl',
    }
    ...
    if lstm_flag == 0:
        opt.state_dict_path = model_state_dict_paths_nolstm[opt.model_name]
    if lstm_flag == 1:
        opt.state_dict_path = model_state_dict_paths_lstm[opt.model_name]
    ...
    t_probs = inf.evaluate('I like screen at night', 'screen')
    # t_probs = inf.evaluate('I like the brightness and adjustments.', 'brightness')
    # t_probs = inf.evaluate('The price is 200 dollars down .', 'price')
    print('Prediction = ', t_probs.argmax(axis=-1)[0])

    # doc = nlp('I like screen at night')
    doc = nlp('I like the laptop but do not like the screen')
